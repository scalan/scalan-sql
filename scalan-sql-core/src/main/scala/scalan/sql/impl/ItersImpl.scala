...
!!!("Cannot convert from CursorIter to TableIter: missing fields List(table, scanId, direction, fakeDep)")
    override def getDefaultRep = TableIter(element[Table].defaultRepValue, 0, element[SortDirection].defaultRepValue, 0)
    override lazy val tag = {
      implicit val tagRow = eRow.tag
  ...
  type TableIterData[Row] = (Table, (Int, (SortDirection, Int)))
  ...
  class TableIterCompanionAbs extends CompanionDef[TableIterCompanionAbs] {
    ...
    def apply[Row](table: Rep[Table], scanId: Rep[Int], direction: Rep[SortDirection], fakeDep: Rep[Int])(implicit eRow: Elem[Row]): Rep[TableIter[Row]] =
  ...
  def mkTableIter[Row](table: Rep[Table], scanId: Rep[Int], direction: Rep[SortDirection], fakeDep: Rep[Int])(implicit eRow: Elem[Row]): Rep[TableIter[Row]]
  def unmkTableIter[Row](p: Rep[CursorIter[Row]]): Option[(Rep[Table], Rep[Int], Rep[SortDirection], Rep[Int])]
  ...
  class IndexIterElem[Row](val iso: Iso[IndexIterData[Row], IndexIter[Row]])(implicit override val eRow: Elem[Row])
    ...
    override def convertCursorIter(x: Rep[CursorIter[Row]]) = // Converter is not generated by meta
!!!("Cannot convert from CursorIter to IndexIter: missing fields List(table, index, scanId, direction, fakeDep)")
    override def getDefaultRep = IndexIter(element[Table].defaultRepValue, element[Index].defaultRepValue, 0, element[SortDirection].defaultRepValue, 0)
    override lazy val tag = {
      implicit val tagRow = eRow.tag
  ...
  // state representation type
  type IndexIterData[Row] = (Table, (Index, (Int, (SortDirection, Int))))
  ...
  class IndexIterCompanionAbs extends CompanionDef[IndexIterCompanionAbs] {
    ...
    @scalan.OverloadId("fromFields")
    def apply[Row](table: Rep[Table], index: Rep[Index], scanId: Rep[Int], direction: Rep[SortDirection], fakeDep: Rep[Int])(implicit eRow: Elem[Row]): Rep[IndexIter[Row]] =
  ...
  // 6) smart constructor and deconstructor
  def mkIndexIter[Row](table: Rep[Table], index: Rep[Index], scanId: Rep[Int], direction: Rep[SortDirection], fakeDep: Rep[Int])(implicit eRow: Elem[Row]): Rep[IndexIter[Row]]
  def unmkIndexIter[Row](p: Rep[CursorIter[Row]]): Option[(Rep[Table], Rep[Index], Rep[Int], Rep[SortDirection], Rep[Int])]
...
trait ItersStd extends ScalanStd with ItersDsl {
  ...
  def mkTableIter[Row]
    (table: Rep[Table], scanId: Rep[Int], direction: Rep[SortDirection], fakeDep: Rep[Int])(implicit eRow: Elem[Row]): Rep[TableIter[Row]] =
  ...
  def mkIndexIter[Row]
    (table: Rep[Table], index: Rep[Index], scanId: Rep[Int], direction: Rep[SortDirection], fakeDep: Rep[Int])(implicit eRow: Elem[Row]): Rep[IndexIter[Row]] =
...
object Iters_Module extends scalan.ModuleInfo {
  val dump = ""
}
}
